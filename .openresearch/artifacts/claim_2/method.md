...
4. Independently construct the full Kronecker Hessian, project it onto the
   diagonal mean-error subspace reachable from the theorem's zero-mean
   initialization, and recover its smallest eigenvalue without using the
   paper's `rho_i` shortcut; also compare the analytical gradient with central
   finite differences. The unrestricted operator is retained as an
   assumption-sensitivity control.
