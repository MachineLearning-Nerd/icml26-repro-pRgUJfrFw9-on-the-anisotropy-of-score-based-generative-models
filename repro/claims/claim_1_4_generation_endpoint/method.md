...
The verifier first requires the frozen one-million-network geometry aggregate
to pass. That aggregate is the equal-weight mean of two independently generated,
disjoint 500,000-network shards using the author architecture and geometry
procedure. The endpoint direction is the deterministic eigenvector of that
full aggregate at the committed index. It reconstructs the author’s
`configs/1x16x16-iddpm.json` `Diffuser` without architectural substitutions.

Cross-shard individual-vector and local-band overlaps are carried into every
endpoint result. The full-estimator direction is never selected or changed
using the generation metric. A direction that fails the preregistered stability
gate remains usable as a literal reproduction of the paper's estimator
procedure, but is explicitly not described as stability-certified.
