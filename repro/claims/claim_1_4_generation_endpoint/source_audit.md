...
untrained architecture under the default zero-input/noise-level probe. The
author geometry script defaults to one million freshly initialized networks;
this node consumes the frozen equal-weight aggregate of two disjoint
500,000-network shards.
...
The two shard estimates agree on the leading eigenvector but do not identify
the smallest or intermediate individual eigenvectors stably. The endpoint
contract therefore separates literal use of the deterministic full-estimator
eigenvector from the stronger, unmet claim that the direction is
stability-certified.

This node is one half of a paired endpoint seed. It cannot by itself verify or
falsify the trend.
