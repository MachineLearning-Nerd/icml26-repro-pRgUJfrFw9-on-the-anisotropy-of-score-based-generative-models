...
GEOMETRY_RAW = (
    ...
    / ".openresearch"
    / "artifacts"
    / "claim_1_4_geometry_million_aggregate"
    / "raw_results.json"
)
...
def load_direction(design: dict) -> tuple[torch.Tensor, dict]:
    ...
    stability = {
        str(row["index"]): row for row in geometry["cross_shard_stability"]
    }
    ...
    return direction, {
        ...
        "geometry_source_status": geometry["scientific_status"],
        "geometry_source_ready": geometry["structural_pass"],
        "geometry_network_count": geometry["network_count"],
        "geometry_source_shards": geometry["source_shards"],
        "geometry_direction_stable": stability[index][
            "individual_direction_ready"
        ],
        "cross_shard_absolute_vector_overlap": stability[index][
            "absolute_vector_overlap"
        ],
        "cross_shard_band_mean_cosine": stability[index]["band_overlap"][
            "mean_cosine"
        ],
    }
...
def main() -> None:
    ...
    result = {
        ...
        "direction_stability_limitation": (
            None
            if direction_record["geometry_direction_stable"]
            else (
                "The deterministic full-estimator direction is faithful to the "
                "paper procedure but did not pass the preregistered cross-shard "
                "individual-direction stability gate."
            )
        ),
    }
    ...
    summary = {
        ...
        "geometry_network_count": direction_record["geometry_network_count"],
        "geometry_direction_stable": direction_record[
            "geometry_direction_stable"
        ],
        "cross_shard_absolute_vector_overlap": direction_record[
            "cross_shard_absolute_vector_overlap"
        ],
