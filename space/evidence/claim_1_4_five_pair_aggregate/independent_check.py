"""Independent reconstruction of the paper-scale five-pair endpoint aggregate."""
...
def main() -> None:
    ...
    checks["paper_scale_geometry"] = source["geometry"]["network_count"] == 1_000_000
    checks["geometry_checker_passed"] = source["geometry"][
        "aggregate_checker_passed"
    ]
    checks["leading_direction_stable"] = source["geometry"][
        "selected_direction_stability"
    ]["0"]["individual_direction_ready"]
    checks["smallest_instability_disclosed"] = not source["geometry"][
        "selected_direction_stability"
    ]["255"]["individual_direction_ready"]
    checks["scoped_status_honest"] = (
        raw["scientific_status"] == "PAPER_SCALE_FIVE_PAIR_ENDPOINT_CORROBORATION"
        and raw["claim_verdict"]
        == "BLOCKED_PENDING_MULTI_DIRECTION_RANK_ASSOCIATION"
    )
